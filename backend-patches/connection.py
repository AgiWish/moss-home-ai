...
class ConnectionHandler:
    ...
    def _fast_home_control(self, query, sentence_id):
        if not query or not self.load_function_plugin:
            return False
        ...
        turn_on_words = ("打开", "开启", "开一下", "开")
        turn_off_words = ("关闭", "关掉", "关上", "关一下", "关")
        action_type = None
        for word in turn_off_words:
            if word in text:
                action_type = "turn_off"
                break
        if action_type is None:
            for word in turn_on_words:
                if word in text:
                    action_type = "turn_on"
                    break
        if action_type is None:
        ...
        service = action_type
        if domain == "cover":
            service = "open_cover" if action_type == "turn_on" else "close_cover"
        ...
        try:
            ...
            self.logger.bind(tag=TAG).warning(f"家控快路径调用HA失败: {type(exc).__name__}: {exc}")
            self._speak_fast_home_result(sentence_id, "网络异常")
            return True

        if response.status_code in (200, 201):
            self.logger.bind(tag=TAG).info(
                f"家控快路径成功: {query} -> {entity_id} {service}"
            )
            self._speak_fast_home_result(sentence_id, "好了")
        else:
            self.logger.bind(tag=TAG).warning(
                f"家控快路径HA返回失败: {entity_id} {service}, status:{response.status_code}, body:{response.text[:200]}"
            )
            self._speak_fast_home_result(sentence_id, "执行失败")
        return True

    def _resolve_fast_home_entity(self, text):
        entity_map = {
            ...
            "电视": "media_player.dian_shi",
            "客厅电视": "media_player.dian_shi",
            ...
            "入户灯": "switch.giot_v6oodm_ef81_switch",
            "过道灯": "switch.giot_v6oodm_ed04_switch",
        }
    ...
    def _speak_fast_home_result(self, sentence_id, text):
    ...
    def chat(self, query, depth=0):
        ...
        if depth == 0:
            ...
            if self._fast_home_control(query, current_sentence_id):
                return True
        else:
